...
def build_embedding(word_to_id):
  ...
  embedding_matrix = np.random.uniform(size=(len(word_to_id) + 10, dimSize), \
    low=-1.0, high=1.9)
...
def guten_producer(raw_data, raw_labels, batch_size, num_steps, name=None):
  ...
  with tf.name_scope(name, "PGProducer", [raw_data, raw_labels, batch_size, \
    ...
    b_l = len(raw_data) // batch_size
    max_index = np.max(raw_data)
    print(len(raw_labels), b_l * batch_size)
